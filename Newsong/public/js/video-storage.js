...
(function(){
  ...
  // Count videos by module (for teachers)
  function countVideosByModule(instrument, level, moduleId){
  ...
  // Count videos by lesson (for teachers)
  function countVideosByLesson(instrument, level, lessonId){
    return new Promise(async (resolve, reject) => {
      try {
        const allVideos = await getAllVideos();
        const count = allVideos.filter(v => 
